/****************************************************************************
** Meta object code from reading C++ file 'TraceWindow.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.9.5)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "window/TraceWindow/TraceWindow.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'TraceWindow.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.9.5. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_TraceWindow_t {
    QByteArrayData data[12];
    char stringdata0[168];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_TraceWindow_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_TraceWindow_t qt_meta_stringdata_TraceWindow = {
    {
QT_MOC_LITERAL(0, 0, 11), // "TraceWindow"
QT_MOC_LITERAL(1, 12, 12), // "rowsInserted"
QT_MOC_LITERAL(2, 25, 0), // ""
QT_MOC_LITERAL(3, 26, 6), // "parent"
QT_MOC_LITERAL(4, 33, 5), // "first"
QT_MOC_LITERAL(5, 39, 4), // "last"
QT_MOC_LITERAL(6, 44, 28), // "on_cbAggregated_stateChanged"
QT_MOC_LITERAL(7, 73, 1), // "i"
QT_MOC_LITERAL(8, 75, 28), // "on_cbAutoScroll_stateChanged"
QT_MOC_LITERAL(9, 104, 38), // "on_cbTimestampMode_currentInd..."
QT_MOC_LITERAL(10, 143, 5), // "index"
QT_MOC_LITERAL(11, 149, 18) // "on_cbFilterChanged"

    },
    "TraceWindow\0rowsInserted\0\0parent\0first\0"
    "last\0on_cbAggregated_stateChanged\0i\0"
    "on_cbAutoScroll_stateChanged\0"
    "on_cbTimestampMode_currentIndexChanged\0"
    "index\0on_cbFilterChanged"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_TraceWindow[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       5,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: name, argc, parameters, tag, flags
       1,    3,   39,    2, 0x0a /* Public */,
       6,    1,   46,    2, 0x08 /* Private */,
       8,    1,   49,    2, 0x08 /* Private */,
       9,    1,   52,    2, 0x08 /* Private */,
      11,    0,   55,    2, 0x08 /* Private */,

 // slots: parameters
    QMetaType::Void, QMetaType::QModelIndex, QMetaType::Int, QMetaType::Int,    3,    4,    5,
    QMetaType::Void, QMetaType::Int,    7,
    QMetaType::Void, QMetaType::Int,    7,
    QMetaType::Void, QMetaType::Int,   10,
    QMetaType::Void,

       0        // eod
};

void TraceWindow::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        TraceWindow *_t = static_cast<TraceWindow *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->rowsInserted((*reinterpret_cast< const QModelIndex(*)>(_a[1])),(*reinterpret_cast< int(*)>(_a[2])),(*reinterpret_cast< int(*)>(_a[3]))); break;
        case 1: _t->on_cbAggregated_stateChanged((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 2: _t->on_cbAutoScroll_stateChanged((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 3: _t->on_cbTimestampMode_currentIndexChanged((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 4: _t->on_cbFilterChanged(); break;
        default: ;
        }
    }
}

const QMetaObject TraceWindow::staticMetaObject = {
    { &ConfigurableWidget::staticMetaObject, qt_meta_stringdata_TraceWindow.data,
      qt_meta_data_TraceWindow,  qt_static_metacall, nullptr, nullptr}
};


const QMetaObject *TraceWindow::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *TraceWindow::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_TraceWindow.stringdata0))
        return static_cast<void*>(this);
    return ConfigurableWidget::qt_metacast(_clname);
}

int TraceWindow::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = ConfigurableWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 5)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 5;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 5)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 5;
    }
    return _id;
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
